...
extern const char *const framebufDate = "$Date: 2014/12/18 16:48:10 $";

double AlphaDataFrameBuffer::baselineFrameRatesInHz[NUM_FRAMERATES] = {
    20.0,    25.0,   33.333, 40.0, 50.0,   66.667, 100.0
};
...
AlphaDataFrameBuffer::AlphaDataFrameBuffer(int fh, void (*er)(const char *s),
	int cc) :
    FrameBuffer(fh, er, cc)
{
    ...
    EFSCResult eRes;
    CJPLADB3CameraCard *cCard;
    CMFBADB3CameraCard *noCCard;
    ...
    *last_error = '\0';
	...
	availableFrameRatesInHz[i] = baselineFrameRatesInHz[i] *
	    (double)(BASELINE_HEIGHT_LINES+4) / (fh+4);
    ...
    if (!m_compressionCapable) {
	m_card = noCCard = new CMFBADB3CameraCard(m_cardIndex, XRM_CLink_DB);
	cCard = NULL;
    }
    else {
	noCCard = NULL;
	m_card = cCard = new CJPLADB3CameraCard(m_cardIndex);
	cCard->SetCaptureMode(CompressedAndRaw);
    }
    ...
    if (cCard)
	m_camera = new CompressionCam(cCard, m_cameraIndex);
    else if (noCCard) {
	m_camera = new CGenericCamera(noCCard, m_cameraIndex,
	    AlphaData::CLink::DATA_FMT_L16,
	    BASE_1X16BITS, FB_PIXEL_WIDTH, m_frameHeight, 0, 0, FB_PIXEL_WIDTH,
	    m_frameHeight, 0, 0);
    ...
    if (!m_compressionCapable) {
	nBankSize = (UINT32)noCCard->GetMemoryBankSize((UINT8)m_cameraIndex);
	nMaxFrame = nBankSize / m_camera->GetFrameSize();
	nMaxFrame /= m_framesPerBuffer;
	noCCard->SetBufferDepth(nMaxFrame);
    }
...
void AlphaDataFrameBuffer::readFPGARegs(int *fpgaFrameCount_p,
    ...
    int *fpgaSerialErrors_p, int *fpgaSerialPortCtl_p)
{
...
{
    return availableFrameRatesInHz[m_currentFrameRate];
}
...
{
    if (selection < 0)
	return availableFrameRatesInHz[0];
    else if (selection >= NUM_FRAMERATES)
	return availableFrameRatesInHz[NUM_FRAMERATES-1];
    else return availableFrameRatesInHz[selection];
}
