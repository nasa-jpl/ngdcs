/* lists.cc -- part of HKCheck, a telemetry verification tool.
   Copyright 1994, by the California Institute of Technology.  ALL RIGHTS
   ...
   Please do not redistribute this file separate from the entire HKCheck source
...
template <class T>
T SL_List<T>::add(T p) throw(std::bad_alloc)
{
    ADD_TO_LIST(T,p)
    return((T)p);
}
...
template <class T>
T SL_List<T>::add(T p,void (*trace_func)(T p,int length)) throw(std::bad_alloc)
{
    ADD_TO_LIST(T,p)
...
template <class T>
T SL_List<T>::add_replace(T p) throw(std::bad_alloc)
{
    int i;
...
template <class T>
T SL_List<T>::remove_current() throw()
{
    T p;
    if (current_scan == -1) {
...
template <class T>
T SL_List<T>::remove_current(void (*trace_func)(T p,int length)) throw()
{
    T p;
    if (current_scan == -1) {
	(void)fprintf(stderr,"remove_current: Internal scanning error\n");
...
template <class T>
T SL_List<T>::remove_previous() throw()
{
    T p;
    if (current_scan == -1) {
...
template <class T>
void SL_List<T>::clear() throw() { current_items = 0; current_scan = -1; }


    // SL_List class -- describe routine

template <class T>
void SL_List<T>::describe(char *notation) throw()
{
...
T *SL_List<T>::get_array()
{
    ...
    (void)memcpy((void *)array,(void *)items,current_items * sizeof(T));
    return(array);
}
...
template <class T>
T SL_List<T>::get_nth(int n) throw()
{
    failed = 0;
...
template <class T>
int SL_List<T>::init_scan(SL_ListWatch<T> *watch) throw()
{
...
template <class T>
T SL_List<T>::next(int (*func)(T p)) throw()
{
    CHECK_INTERNALS(T)
...
template <class T>
T SL_List<T>::next(int (*func)(T p,void *p1),void *p1) throw()
{
    CHECK_INTERNALS(T)
    while (scan_pts[current_scan] < current_items &&
...
template <class T>
T SL_List<T>::next(int (*func)(T p,void *p1,void *p2),void *p1,void *p2) throw()
{
    CHECK_INTERNALS(T)
    while (scan_pts[current_scan] < current_items &&
...
template <class T>
T SL_List<T>::find_first(int (*func)(T p)) throw()
{
    INIT(T)
...
template <class T>
T SL_List<T>::find_first(int (*func)(T p,void *p1),void *p1) throw()
{
    INIT(T)
    ...
    throw()
{
    ...
    // SL_List class -- find-two routine

template <class T>
T SL_List<T>::find_two(int (*func1)(T p1), int (*func2)(T p1, T p2), T *other)
    throw()
{
    int sp1 = 0;
    int sp2 = 0;
    ...
    failed = 0;
    if (current_items != 0)
	for (sp1 = 0;sp1 < current_items-1;sp1++)
	    if ((*func1)(items[sp1]))
		for (sp2 = sp1+1;sp2 < current_items;sp2++)
		    if ((*func2)(items[sp1],items[sp2])) {
			*other = items[sp2];
			return (T)items[sp1];
		    }
    return (T)NULL;
...
template <class T>
T SL_List<T>::remove_first() throw()
{
    T p;
    INIT(T)
    REMOVE(T,sp,p)
...
template <class T>
T SL_List<T>::remove_first(int (*func)(T p)) throw()
{
    T p;
    INIT(T)
    while (!(*func)(items[sp]) && ++sp < current_items) ;
...
template <class T>
T SL_List<T>::remove_first(int (*func)(T p,void *p1),void *p1) throw()
{
    T p;
    INIT(T)
    while (!(*func)(items[sp],p1) && ++sp < current_items) ;
...
template <class T>
T SL_List<T>::remove_first(void (*trace_func)(T p,int length)) throw()
{
    T p;
    INIT(T)
    REMOVE(T,sp,p)
    ...
    void (*trace_func)(T p,int length)) throw()
{
    ...
    // SL_List class -- remove-last routines

template <class T>
T SL_List<T>::remove_last() throw()
{
    T p;
    INIT(T)
    REMOVE(T,current_items-1,p)
    return((T)p);
}
...
template <class T>
T SL_List<T>::insert(T p) throw(std::bad_alloc)
{
    INSERT(T,p,0)
    return((T)p);
}
...
template <class T>
T SL_List<T>::insert(T p,int (*func)(T orig,T p)) throw(std::bad_alloc)
{
    int i,status=0;
...
template <class T>
T SL_List<T>::insert(T p,int (*func)(T orig,T p,void *p1,void *p2),void *p1,
    void *p2,void (*trace_func)(T p,int length)) throw(std::bad_alloc)
{
    int i,status=0;
    for (i=0;i < current_items;i++) if (status=(*func)(items[i],p,p1,p2)) break;
    if (status != 2) INSERT(T,p,i)
...
template <class T>
T SL_List<T>::insert_before_current(T p) throw(std::bad_alloc)
{
    if (current_scan == -1) {
...
template <class T>
SL_ListWatch<T>::SL_ListWatch() throw() { list = NULL; }


template <class T>
void SL_ListWatch<T>::set_list(SL_List<T> *l) throw() { list = l; }


template <class T>
SL_ListWatch<T>::~SL_ListWatch() throw() { if (list) list->scan_done(); }
